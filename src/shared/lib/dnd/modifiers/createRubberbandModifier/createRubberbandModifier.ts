...
// Initially used for mobile drawer, took from example of dnd-kit
// https://github.com/clauderic/dnd-kit/blob/master/stories/3%20-%20Examples/Drawer/modifiers.ts

export const createRubberbandModifier = (maxHeightPercent: number): Modifier => ({
  draggingNodeRect,
  transform,
  windowRect,
}) => {
  ...
  return rubberbandBoundingRect(transform, draggingNodeRect, {
    ...windowRect,
    top: (1 - maxHeightPercent) * windowRect.height,
    height: maxHeightPercent * windowRect.height,
  });
};
...
function rubberbandBoundingRect (
  ...
  if (rect.top + transform.y <= boundingRect.top) {
    ...
    rect.bottom + transform.y >= boundingRect.top + boundingRect.height
  ) {
